...
e2e-test-core: build-image ensure-helm
	...
	@build/setup-import-controller.sh
	go test -c ./test/e2e -o _output/e2e.test
	_output/e2e.test -test.v -ginkgo.v --ginkgo.label-filter="core && !agent-registration" --ginkgo.timeout=60m
...
e2e-test-misc: build-image ensure-helm
	...
	@build/setup-import-controller.sh
	go test -c ./test/e2e -o _output/e2e.test
	_output/e2e.test -test.v -ginkgo.v --ginkgo.label-filter="!core && !hosted && !agent-registration" --ginkgo.timeout=60m
