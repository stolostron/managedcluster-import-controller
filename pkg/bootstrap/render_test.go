...
func TestKlusterletConfigGenerate(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			expectError:  true,
			errorMessage: "failed to get GRPC config yaml for klusterlet registration driver",
			validateFunc: func(t *testing.T, objects, crds []runtime.Object) {
...
func TestBuildGRPCConfigData(t *testing.T) {
	testcases := []struct {
	...
	}{
		{
			...
			expectedGRPCConfig: "keepAliveConfig: {}\ntoken: test-token\nurl: grpc.config.com\n",
		},
		{
	...
	for _, testcase := range testcases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			grpcConfigData, err := buildGRPCConfigData(clientHolder, testcase.token)
			if testcase.expectError {
				if err == nil {
