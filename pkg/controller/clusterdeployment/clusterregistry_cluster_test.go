...
func Test_newClusterRegistryCluster(t *testing.T) {
	type args struct {
		clusterDeployment *hivev1.ClusterDeployment
	}
	testClusterDeployment := &hivev1.ClusterDeployment{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, err := newClusterRegistryCluster(tt.args.clusterDeployment); !reflect.DeepEqual(got, tt.want) {
				if (err != nil) != tt.wantErr {
					t.Errorf("newClusterRegistryCluster() error = %v, wantErr %v", err, tt.wantErr)
					return
				}
				t.Errorf("newClusterRegistryCluster() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func Test_createClusterRegistryCluster(t *testing.T) {
	...
	fclient := fake.NewFakeClientWithScheme(fscheme, testClusterDeployment)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want == nil {
				...
				if got.ClusterName != tt.want.GetClusterName() || got.Namespace != tt.want.GetNamespace() {
					t.Errorf("createClusterRegistryCluster() = %v, want %v", got, tt.want)
				}
			}
		})
	}
