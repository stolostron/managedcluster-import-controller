...
func (r *ReconcileManagedCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if len(managedCluster.Finalizers) == 0 || managedCluster.Finalizers[0] != importFinalizer {
	...
	return reconcile.Result{}, helpers.RemoveManagedClusterFinalizer(r.client, r.recorder, managedCluster, importFinalizer)
}

func (r *ReconcileManagedCluster) ensureManagedClusterMetaObj(
	...
	helpers.AddManagedClusterFinalizer(modified, managedCluster, importFinalizer)
	if *modified {
...
func (r *ReconcileManagedCluster) deleteManagedClusterNamespace(
	...
	pods := &corev1.PodList{}
	err = r.client.List(ctx, pods, client.InNamespace(clusterName))
	if err != nil {
		return err
	}
	for _, pod := range pods.Items {
		if !strings.HasPrefix(pod.Name, curatorJobPrefix) &&
			...
			r.recorder.Warningf("ManagedClusterNamespaceNotDeleted", "There are non curator pods in namespace %s", clusterName)
			return nil
		}
	}
