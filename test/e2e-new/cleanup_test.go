...
var _ = ginkgo.Describe("test cleanup resource after a cluster is detached", ginkgo.Label("cleanup"), func() {
	ginkgo.Context("Importing a self managed cluster and detach the cluster", func() {
		...
		ginkgo.It("Should delete addons and manifestWorks", func() {
			...
			_, err := hub.WorkClient.WorkV1().ManifestWorks(localClusterName).Create(context.TODO(), manifestwork, metav1.CreateOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())

			// check the work has added finalizer before detaching the cluster
			hub.AssertManifestWorkFinalizer(localClusterName, manifestwork.Name, "cluster.open-cluster-management.io/manifest-work-cleanup")
			...
			_, err = hub.AddonClient.AddonV1alpha1().ManagedClusterAddOns(localClusterName).Create(context.TODO(), addon, metav1.CreateOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())

			ginkgo.By(fmt.Sprintf("wait for the 2 finalizers to be applied for cluster %s", localClusterName))
			gomega.Eventually(func() bool {
				cluster, err := hub.ClusterClient.ClusterV1().ManagedClusters().Get(context.TODO(), localClusterName, metav1.GetOptions{})
				if err != nil {
					return false
				}
			...
			// DeleteCluster handles leader election via EnsureAgentReady before deleting
			cl.DeleteCluster()

			// the addon and manifestWork should be deleted.
			gomega.Eventually(func() error {
				addons, err := hub.AddonClient.AddonV1alpha1().ManagedClusterAddOns(localClusterName).List(context.TODO(), metav1.ListOptions{})
				if err != nil {
			...
			gomega.Eventually(func() error {
				allManifestWorks, err := hub.WorkClient.WorkV1().ManifestWorks(localClusterName).List(context.TODO(), metav1.ListOptions{})
				if err != nil {
					if errors.IsNotFound(err) {
						return nil
					}
			...
			gomega.Eventually(func() error {
				_, err := hub.ClusterClient.ClusterV1().ManagedClusters().Get(context.TODO(), localClusterName, metav1.GetOptions{})
				if errors.IsNotFound(err) {
					return nil
				}
				if err == nil {
			...
			gomega.Eventually(func() error {
				_, err := hub.KubeClient.CoreV1().Namespaces().Get(context.TODO(), localClusterName, metav1.GetOptions{})
				if errors.IsNotFound(err) {
					return nil
				}
				if err == nil {
		...
		ginkgo.It("should keep the ns when infraenv exists", func() {
			...
			_, err = hub.DynamicClient.Resource(infraGVR).Namespace(managedClusterName).Create(context.TODO(), unstructuredInfra, metav1.CreateOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			...
			err = hub.DynamicClient.Resource(infraGVR).Namespace(managedClusterName).Delete(context.TODO(), infra.Name, metav1.DeleteOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
