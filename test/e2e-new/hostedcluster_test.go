...
var _ = ginkgo.Describe("Importing and detaching a managed cluster with hosted mode", ginkgo.Label("hosted"), func() {
	...
	var hostingCl *framework.ClusterLifecycle
	...
	ginkgo.BeforeEach(func() {
		...
		ginkgo.By(fmt.Sprintf("Create hosting cluster %s", hostingClusterName), func() {
			_, err := util.CreateManagedCluster(hub.ClusterClient, hostingClusterName,
				util.NewLable("local-cluster", "true"))
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
		})

		hostingCl = framework.ForDefaultMode(hub, hostingClusterName)

		hub.AssertImportSecretCreated(hostingClusterName, "other")
		hub.AssertImportSecretApplied(hostingClusterName)
		hub.AssertClusterAvailable(hostingClusterName)
		hub.AssertManifestWorksAvailable(hostingClusterName)
	})
	...
	ginkgo.Context("Import one hosted managed cluster with auto-import-secret", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			cl = framework.ForHostedMode(hub, managedClusterName, hostingClusterName)
		...
		ginkgo.It("Should import the cluster with auto-import-secret with kubeconfig", func() {
			ginkgo.By(fmt.Sprintf("Create auto-import-secret for managed cluster %s with kubeconfig", managedClusterName), func() {
				secret, err := util.NewAutoImportSecret(hub.KubeClient, managedClusterName, operatorv1.InstallModeHosted)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
			...
			ginkgo.By(fmt.Sprintf("Create hosted mode managed cluster %s", managedClusterName), func() {
				_, err := util.CreateHostedManagedCluster(hub.ClusterClient, managedClusterName, hostingClusterName)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
			})

			hub.AssertImportSecretCreated(managedClusterName, "other", operatorv1.InstallModeHosted)
			hub.AssertImportSecretApplied(managedClusterName, operatorv1.InstallModeHosted)
			hub.AssertClusterAvailable(managedClusterName)
			hub.AssertHostedManifestWorksAvailable(managedClusterName, hostingClusterName)
			hub.AssertPriorityClassHosted(managedClusterName)
		})
	})
	...
	ginkgo.Context("Import one hosted managed cluster manually", func() {
		...
		ginkgo.It("Should import the cluster by creating the external managed kubeconfig secret manually", func() {
			...
			ginkgo.By(fmt.Sprintf("Create external managed kubeconfig %s with kubeconfig", managedClusterName), func() {
				secret, err := util.NewExternalManagedKubeconfigSecret(hub.KubeClient, managedClusterName)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())

				spokeKubeclient, err := util.NewManagedClient(hub.KubeClient)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())

				namespace := fmt.Sprintf("klusterlet-%s", managedClusterName)
				hub.AssertNamespaceCreated(hub.KubeClient, namespace)
				_, err = hub.KubeClient.CoreV1().Secrets(namespace).Create(
				...
				hub.AssertNamespaceCreated(spokeKubeclient, spokeNamespace)
			})
		...
		ginkgo.It("Should override the klusterlet namespace by the annotation", func() {
			...
			ginkgo.By(fmt.Sprintf("Create external managed kubeconfig %s with kubeconfig", managedClusterName), func() {
				...
				hub.AssertNamespaceCreated(spokeKubeclient, "open-cluster-management-agent-hosted")
			})
	...
	ginkgo.Context("Detach multiple hosted clusters", func() {
		ginkgo.It("should delete each cluster independently", func() {
			...
			cl1 := framework.ForHostedMode(hub, hosted1, hostingClusterName)
			...
			ginkgo.By(fmt.Sprintf("Create hosted mode managed cluster %s", hosted1), func() {
				_, err := util.CreateHostedManagedClusterWithShortLeaseDuration(hub.ClusterClient, hosted1, hostingClusterName)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
			})

			hub.AssertImportSecretCreated(hosted1, "other", operatorv1.InstallModeHosted)
			hub.AssertHostedManifestWorks(hostingClusterName, hosted1)

			// create another hosted cluster
			hosted2 := fmt.Sprintf("test-hosted-%s", rand.String(6))
			cl2 := framework.ForHostedMode(hub, hosted2, hostingClusterName)
			...
			ginkgo.By(fmt.Sprintf("Create hosted mode managed cluster %s", hosted2), func() {
				_, err := util.CreateHostedManagedClusterWithShortLeaseDuration(hub.ClusterClient, hosted2, hostingClusterName)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
			})

			hub.AssertImportSecretCreated(hosted2, "other", operatorv1.InstallModeHosted)
			hub.AssertHostedManifestWorks(hostingClusterName, hosted2)
	...
	ginkgo.Context("Cleanup resources after a hosted cluster is detached", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			ginkgo.By(fmt.Sprintf("Create hosted mode managed cluster %s", managedClusterName), func() {
				_, err := util.CreateHostedManagedClusterWithShortLeaseDuration(hub.ClusterClient, managedClusterName, hostingClusterName)
				gomega.Expect(err).ToNot(gomega.HaveOccurred())
			})
		...
		ginkgo.It("Should clean up the addons", func() {
			hub.AssertClusterNamespace(managedClusterName)
			// deploy an addon
			...
			_, err := hub.AddonClient.AddonV1alpha1().ManagedClusterAddOns(managedClusterName).Create(context.TODO(), addon, metav1.CreateOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			...
			gomega.Eventually(func() error {
				_, err := hub.AddonClient.AddonV1alpha1().ManagedClusterAddOns(managedClusterName).Get(context.TODO(), addon.Name, metav1.GetOptions{})
				if err != nil {
		...
		ginkgo.It("Should clean up the addons with finalizer", func() {
			...
			_, err = hub.WorkClient.WorkV1().ManifestWorks(managedClusterName).Create(context.TODO(), manifestwork, metav1.CreateOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			...
			gomega.Eventually(func() error {
				_, err := hub.WorkClient.WorkV1().ManifestWorks(managedClusterName).Get(context.TODO(), manifestwork.Name, metav1.GetOptions{})
				if err != nil {
					if errors.IsNotFound(err) {
						return nil
					}
