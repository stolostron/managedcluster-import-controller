...
var _ = ginkgo.Describe("Using customized image registry", ginkgo.Label("config"), func() {
	...
	ginkgo.BeforeEach(func() {
		...
		// No agent is deployed in this test; only the ManagedCluster resource
		// and import secret are created.
		cl = framework.ForCreatedOnly(hub, managedClusterName)

		ginkgo.By(fmt.Sprintf("Create pull secret %s", pullSecretName), func() {
			err := util.CreatePullSecret(hub.KubeClient, "default", pullSecretName)
		...
		ginkgo.By(fmt.Sprintf("Create managed cluster %s", managedClusterName), func() {
			...
			_, err := util.CreateClusterWithImageRegistries(hub.ClusterClient, managedClusterName, imageRegistries)
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
		})
	})
	...
	ginkgo.AfterEach(func() {
		cl.Teardown()
		hub.AssertPullSecretDeleted("default", pullSecretName)
	})
	...
	ginkgo.It("Should using customized image registry", func() {
		...
		ginkgo.By("Check image registry", func() {
			...
			secret, err := hub.KubeClient.CoreV1().Secrets(managedClusterName).Get(context.TODO(), name, metav1.GetOptions{})
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
